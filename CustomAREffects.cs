...
public class CustomAREffects : DefaultObserverEventHandler
{
    ...
    private IEnumerator MoveWebTowardsTarget(GameObject web)
    {
        ...
        float initialScaleZ = web.transform.localScale.z; // initial scale in y direction
        ...
        while ((Time.time - startTime) < journeyDuration)
        {
            ...
            // Stretch the web: scale it based on the distance it has moved from its start position.
            float stretchFactor = (web.transform.position - startPosition).magnitude;
            web.transform.localScale = new Vector3(web.transform.localScale.x, web.transform.localScale.y, initialScaleZ + stretchFactor);
    ...
    /*private IEnumerator MoveWebTowardsTarget(GameObject web)
    {
        ...
        // Define your minimum and maximum scales here:
        float minScale = 0.01f; // Scale when the web is near the camera

        float maxScale = 5f;   // Scale when the web has reached its target position
        ...
        while ((Time.time - startTime) < journeyDuration)
        {
            ...
            // Calculate the current fraction of the journey completed
            float journeyFraction = (web.transform.position - startPosition).magnitude / (endPosition - startPosition).magnitude;

            // Lerp the scale based on journey fraction for overall size
            float lerpScale = Mathf.Lerp(minScale, maxScale, journeyFraction);

            // Calculate stretch in the Z-axis based on journey fraction
            //float stretchZ = lerpScale + journeyFraction * (endPosition - startPosition).magnitude;

            float stretchZ = lerpScale * (1 + journeyFraction);


            // Set the scale
            web.transform.localScale = new Vector3(lerpScale, lerpScale, stretchZ);
